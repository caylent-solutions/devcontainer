# [S2.2.2] devcontainer.json Image Reference Updates
...
Update the base image reference in both devcontainer.json files from the Microsoft Container Registry (MCR) URL to the ECR Public mirror URL. Both files use the same base image and must be kept in sync by the mirror workflow.

## Definition of Ready

- S2.2.1 (GitHub Actions workflow) must be complete
- S1.4.2 (catalog repo structure — need `collections/default/devcontainer.json`) must be complete
...
Where `<ALIAS>` is the ECR Public registry alias assigned during S2.1.1.
...
- [ ] `.devcontainer/devcontainer.json` image updated to ECR Public URL
- [ ] `collections/default/devcontainer.json` image updated to ECR Public URL
- [ ] Both files reference the same ECR Public image
