# [S1.5.3] project-setup.sh Lifecycle
...
Implement the project-setup.sh lifecycle: the template version lives in common/devcontainer-assets/, gets copied to projects during setup, is sourced by the shared postcreate script at container startup, and is overwritten on re-setup along with all other devcontainer files.
...
- S1.5.1 (setup-devcontainer catalog integration) must be complete

## Full Requirements

### Template Version

`common/devcontainer-assets/project-setup.sh` is the template version that ships with each catalog. It serves as the starting point for project-specific customization.

### Copy During Setup

When setup-devcontainer copies files to a project, `project-setup.sh` is included as part of the collection files copied by `copy_collection_to_project()`.

### Postcreate Script Integration

The shared postcreate script calls `project-setup.sh` if it exists:

```bash
if [ -f "${WORK_DIR}/.devcontainer/project-setup.sh" ]; then
    source "${WORK_DIR}/.devcontainer/project-setup.sh"
fi
```

This allows developers to add project-specific setup steps (installing additional tools, configuring project-specific environment, etc.) that run automatically when the devcontainer is created.

### Developer Customization

After initial setup, the developer edits `project-setup.sh` for project-specific customization. This is the intended customization point for project-level needs.

### Re-setup Behavior

On re-setup (when user chooses to replace .devcontainer/ files), `project-setup.sh` is overwritten along with all other devcontainer files. The replacement notification already instructs the developer to merge back customizations.

### Key Files

- `common/devcontainer-assets/project-setup.sh`
