# [S1.6.1] Caylent Internal Catalog Repo
...
Create the Caylent internal DevContainer catalog repository that serves all Caylent engineering practices (CAE, CNA, CDE, and Solutions). The catalog follows the same structure as this repo's catalog layout and is fully self-contained with its own common/devcontainer-assets/.
...
- **Manual prerequisite: human must create `caylent-solutions/caylent-devcontainer-catalog` GitHub repo before this story can start**

## Full Requirements

### Catalog Structure

Create the following structure in the `caylent-devcontainer-catalog` repo:
...
caylent-devcontainer-catalog/
  ...
  common/
    devcontainer-assets/
      .devcontainer.postcreate.sh       <- Shared postcreate (can differ from default)
...
The Caylent catalog has its OWN `common/devcontainer-assets/` — independent of the default catalog. Each catalog is self-contained. The postcreate script, shared functions, and project-setup template can be customized for Caylent-specific needs without affecting the default catalog.

### Documentation

- `README.md` — Comprehensive catalog documentation covering structure, usage, and maintenance
- `CONTRIBUTING.md` — How to add and maintain collections, PR process, validation requirements, naming conventions

### Validation

The catalog must pass validation using `cdevcontainer catalog validate --local .`
