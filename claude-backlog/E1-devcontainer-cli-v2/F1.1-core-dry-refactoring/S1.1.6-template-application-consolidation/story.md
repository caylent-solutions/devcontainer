# [S1.1.6] Template Application Consolidation
...
Merge duplicate template application and interactive setup functions into single unified implementations. The merged functions use shared utilities from S1.1.2, UI utilities from S1.1.3, and write_project_files() from S1.1.5 to eliminate all duplication in template processing and file generation flows.
...
- S1.1.5 must be complete (depends on write_project_files)
- S1.1.2 must be complete (depends on get_template_path, get_template_names, etc.)
- S1.1.3 must be complete (depends on ask_or_exit, exit_cancelled, exit_with_error)

## Full Requirements

### C. Template Application (Spec Section C)

**Merge `apply_template()` and `apply_template_without_clone()`** into one function:

`apply_template(template_data, target_path)`:
- Handles only template data processing and environment file generation via `write_project_files()`
- Does NOT copy `.devcontainer/` files — that responsibility belongs to the catalog pipeline's `copy_collection_to_project()` (see Feature F1.4)
- Must use shared utilities from S1.1.2 (get_template_path, get_template_names, etc.)
- Must use shared UI utilities from S1.1.3 (ask_or_exit, exit_cancelled, exit_with_error)
- Must use `write_project_files()` from S1.1.5 for all file generation
- Handles both "new setup" and "existing setup" paths through the same code path
- Old duplicate functions removed entirely after merge

**Merge `interactive_setup()` and `interactive_setup_without_clone()`** into one function:

`interactive_setup(target_path)`:
- Handles the interactive template selection/creation flow
- Calls `apply_template()` for file generation
- Must use shared utilities from S1.1.2 (get_template_path, get_template_names, etc.)
- Must use shared UI utilities from S1.1.3 (ask_or_exit, exit_cancelled, exit_with_error)
- Must use `write_project_files()` from S1.1.5 for all file generation
- Handles both "new setup" and "existing setup" paths through the same code path
- Old duplicate functions removed entirely after merge
- Does NOT contain any `.devcontainer/` file copying logic (that moves to catalog pipeline)

### Key Files to Modify

- `caylent-devcontainer-cli/src/caylent_devcontainer_cli/commands/setup.py` — merge apply_template functions
- `caylent-devcontainer-cli/src/caylent_devcontainer_cli/commands/setup_interactive.py` — merge interactive_setup functions
- `caylent-devcontainer-cli/src/caylent_devcontainer_cli/utils/fs.py` — uses write_project_files()
- `caylent-devcontainer-cli/tests/unit/` — add/update unit tests
- `caylent-devcontainer-cli/tests/functional/` — add/update functional tests
