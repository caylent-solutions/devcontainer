# [S1.4.2] Refactor Repo to Catalog Structure
...
Add catalog structure alongside existing repo contents. .devcontainer/ is UNTOUCHED — it is this repo's own dev environment.
...
**New Structure:**
```
.devcontainer/                          <- UNCHANGED — NOT part of catalog
...
common/                                 <- NEW — shared catalog assets
  devcontainer-assets/
    ...
    project-setup.sh                    <- distributable template project-setup
collections/                            <- NEW — catalog collections
  default/
    catalog-entry.json                  <- metadata marker
    devcontainer.json                   <- distributable devcontainer config
...
- .devcontainer/ is NOT part of catalog (exists solely for developing this repo)
- common/ and collections/default/ are distributable catalog content
- Files in common/ and collections/default/ MAY differ from .devcontainer/ files
- CLI clones this repo as default catalog, scans for catalog-entry.json, finds collections/default/, ignores everything else
...
### CLI Constant (utils/constants.py)
...
- `common/` (new directory) — shared catalog assets
- `collections/` (new directory) — catalog collections
- `caylent-devcontainer-cli/src/caylent_devcontainer_cli/utils/constants.py` — add DEFAULT_CATALOG_URL
