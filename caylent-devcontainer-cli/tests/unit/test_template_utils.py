...
class TestCheckTemplateVersion:
    """Tests for check_template_version()."""

    def test_accepts_matching_major_version(self):
        """Test that a template with matching major version is accepted."""
        from caylent_devcontainer_cli.utils.template import check_template_version

        template_data = {"cli_version": "2.0.0", "containerEnv": {}}
        with patch("caylent_devcontainer_cli.utils.template.__version__", "2.1.0"):
            # Should not raise
            check_template_version(template_data)

    def test_rejects_template_without_cli_version(self):
        """Test that a template without cli_version is rejected."""
        from caylent_devcontainer_cli.utils.template import check_template_version

        template_data = {"containerEnv": {}}
        with patch("caylent_devcontainer_cli.utils.template.__version__", "2.0.0"):
            with pytest.raises(SystemExit):
                check_template_version(template_data)

    def test_rejects_v1_template(self):
        """Test that a v1.x template is rejected when running v2.x CLI."""
        from caylent_devcontainer_cli.utils.template import check_template_version

        template_data = {"cli_version": "1.14.1", "containerEnv": {}}
        with patch("caylent_devcontainer_cli.utils.template.__version__", "2.0.0"):
            with pytest.raises(SystemExit):
                check_template_version(template_data)

    def test_rejects_future_major_version(self):
        """Test that a template from a future major version is rejected."""
        from caylent_devcontainer_cli.utils.template import check_template_version

        template_data = {"cli_version": "3.0.0", "containerEnv": {}}
        with patch("caylent_devcontainer_cli.utils.template.__version__", "2.0.0"):
            with pytest.raises(SystemExit):
                check_template_version(template_data)

    def test_accepts_same_version(self):
        """Test that a template with exact same version passes."""
        from caylent_devcontainer_cli.utils.template import check_template_version
    ...
    def test_rejects_invalid_version_string(self):
        """Test that an invalid version string in template is rejected."""
        from caylent_devcontainer_cli.utils.template import check_template_version

        template_data = {"cli_version": "not-a-version", "containerEnv": {}}
        with patch("caylent_devcontainer_cli.utils.template.__version__", "2.0.0"):
            with pytest.raises(SystemExit):
                check_template_version(template_data)
